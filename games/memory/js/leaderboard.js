...
import { levelSel, themeSel, player1, moves} from './script.js';
...
/* ---------- Leaderboard display ---------- */
export function saveToLeaderboard(moves, hrs, min, sec) {
    console.log("saveToLeaderboard()")
    const level = levelSel.value;
    const theme = themeSel.value;
    const elapsed = `${hrs}:${min}:${sec}`
    const dateTime = new Date().toLocaleString();
    const entry = { player1, level, theme, moves, elapsed, dateTime };
    console.log("saveToLeaderboard()", entry)
...
export function toggleLeaderboard() {
    ...
    if (data.length === 0) {
        ...
        list.innerHTML = `<table><thead><tr><th>Winner</th><th>Level</th><th>Theme</th><th>moves</th><th>Elapsed Time</th><th>date time</th></tr></thead><tbody>${data.map(entry => `<tr><td>${entry.player1}</td><td>${entry.level}</td><td>${entry.theme}</td><td>${entry.moves}</td><td>${entry.elapsed}</td><td>${entry.dateTime}</td></tr>`).join('')}</tbody></table>`;
    }
